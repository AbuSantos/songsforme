...
contract BullchordMarketPlace is ReentrancyGuard {
    ...
    event RoyaltySent(address indexed royaltyRecipient, uint256 amount);
    ...
    mapping(address => uint256) public fallbackRoyalties;
    ...
    function buyBull(
        ...
        if (royalty > 0) {
            ...
            if (!royaltySuccess) {
                fallbackRoyalties[royaltyRecipient] += royalty;
            } else {
    ...
    function acceptOffer(
        ...
        if (isItemStaked[_tokenId]) {
         ...
         // Deduct platform fee from amountToSeller after royalty deduction
        uint256 _platformFee = calculatePlatformFee(amountToSeller);
        amountToSeller -= _platformFee;
        listingProceeds += _platformFee;
    ...
    function claimRoyalties() public {
        uint256 amount = fallbackRoyalties[msg.sender];
        require(amount > 0, "No royalties to claim");
        fallbackRoyalties[msg.sender] = 0;
